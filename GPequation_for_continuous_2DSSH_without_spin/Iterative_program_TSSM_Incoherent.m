function [Result_matrix,nR,Number,Number_boundary,m,T_time] = Iterative_program_TSSM_Incoherent(a0,nx,ny,Boundary,Boundarx,sitea,dT,Cord,Step,Space_X,Space_Y,Vpo,Em,Pump,PumpVortex1,PumpVortex2,Vortex0,Pulse_P,F0,kx,Omega_F,g_P,g_nR,R,gamma_P,gamma_nR,tau)
...
T=0;
% excitedsite=1;
PumpVortexconj1=conj(PumpVortex1);
PumpVortexconj2=conj(PumpVortex2);
...
PumpVortex1_gpu=gpuArray(single(PumpVortex1));
PumpVortexconj1_gpu=gpuArray(single(PumpVortexconj1));
PumpVortex2_gpu=gpuArray(single(PumpVortex2));
PumpVortexconj2_gpu=gpuArray(single(PumpVortexconj2));
...
%     Middle_Matrix=Middle_Matrix+(0.5*dT*exp(-((T-T0)/tau)^2))*PumpVortex_gpu.*exp(-1j*Omega_F*T);
    ...
    if T<T1-tau
        ...
        C_vortex=(Vortex0*(P1*exp(-((T-T0)/tau)^2)+P0)*dx*dy*dT)*sum(sum(PumpVortexconj1_gpu.*Middle_Matrix));
%         Middle_Matrix=Middle_Matrix+C_vortex*PumpVortex_gpu*dT;
        Middle_Matrix=Middle_Matrix+C_vortex*Pulse.*PumpVortex1_gpu;
    else
        Pulse=0.1*Pulse_P_gpu*exp(-((T-T1)/tau)^2)+Pump_gpu;
        C_vortex=(Vortex0*(P1*exp(-((T-T1)/tau)^2)+P0)*dx*dy*dT)*sum(sum(PumpVortexconj2_gpu.*Middle_Matrix));
%         Middle_Matrix=Middle_Matrix+C_vortex*PumpVortexconj_gpu*dT;
        Middle_Matrix=Middle_Matrix+C_vortex*Pulse.*PumpVortex2_gpu;
    end
